...
import { cart, userLogo, vyaparLogo } from "../assets/index";
...
function Header() {
  ...
  const productData = useSelector((state) => state.vyapar.productData.length);
  const userInfo = useSelector((state) => state.vyapar.userInfo);
  return (
    <div className="w-full h-20 bg-white border-b-[1px] sticky top-0 z-50 border-b-gray-800 font-titleFont">
      <div className="max-w-screen-xl h-full mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <Link to="/" className="shrink-0">
          <img className="w-20 sm:w-28" src={vyaparLogo} alt="vyapar Logo" />
        </Link>
        <div className="hidden sm:flex sm:items-center sm:gap-8">
          ...
          <Link to="/cart">
            <div className="relative w-16">
              <img className="w-full h-auto" src={cart} alt="cart" />
              ...
              <ul className="flex flex-col items-center space-y-4">
                ...
                <li className="relative">
                  <Link to="/cart" className="flex items-center">
                    <img className="w-20 h-16" src={cart} alt="cart" />
